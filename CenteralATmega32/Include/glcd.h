...
void trigger()
{
   PORTC|=1<<EN; //EN = 1; //EN high
   
   _delay_us(E_DELAY);
   PORTC&=~(1<<EN);//EN = 0; //EN low
   _delay_us(E_DELAY);
}
//----------------------
void glcd_on()
{
    //Activate both chips
        ...
        PORTC|=1<<CS2;//CS2 = 1;
    #endif
    PORTD&=~(1<<RS);//RS = 0;          //RS low --> command
    PORTC&=~(1<<RW);//RW = 0;          //RW low --> write
...
void goto_col(unsigned int x)
{
   ...
   {
      #ifdef CS_ACTIVE_LOW
        PORTC&=~(1<<CS1);//CS1 = 0;          //select chip 1
        PORTC|=1<<CS2;//CS2 = 1;          //deselect chip 2
      #else          
        PORTC|=1<<CS1;//CS1 = 1;          //select chip 1
        PORTC&=~(1<<CS2);//CS2 = 0;          //deselect chip 2
      #endif
   ...
   else                 //right section
   {
        ...
        PORTC|=1<<CS1;//CS1 = 1;
      #else
        PORTC|=1<<CS2;//CS2 = 1;          //select chip 2
        PORTC&=~(1<<CS1);//CS1 = 0;          //deselct chip 1
      #endif
...
byte is_busy()
{
    ...
    PORTC&=~(1<<EN);//EN = 0;                 //Low Enable
    _delay_us(1);            //tf
    PORTC|=1<<RW;//RW = 1;                 //Read
    PORTD&=~(1<<RS);//RS = 0;                 //Status         
    _delay_us(1);            //tasu
    PORTC|=1<<EN;//EN = 1;                 //High Enable
    _delay_us(5);            //tr + fmax(td,twh)->twh
...
byte glcd_read(byte column)
{
    ...
    #ifdef CS_ACTIVE_LOW
        PORTC = (column>63)? PORTC | 1<<CS1 : PORTC & ~(1<<CS1);
    #else         
        PORTC = (column<64)  ? PORTC | 1<<CS1 : PORTC & ~(1<<CS1);//Enable/Disable CS1
    #endif
	PORTC = (PORTC &(1<<CS1)) ? PORTC & ~(1<<CS2) : PORTC | 1<<CS2; //CS2 = !CS1;         //Disable/Enable CS2
    _delay_us(1);        //tasu
    PORTC|=1<<EN;//EN = 1;             //Latch RAM data into ouput register
    _delay_us(1);        //twl + tf
    ...
    PORTC|=1<<EN;//EN = 1;             //latch data from output register to data bus
    _delay_us(1);        //tr + td(twh)       
                                  
    read_data = DATAPIN;    //Input data    
    PORTC&=~(1<<EN);//EN = 0;             //Low Enable to remove data from the bus 
    _delay_us(1);        //tdhr
    #ifdef DEBUG_READ
        printf("R:%x\n\r",read_data);
    #endif
...
void putItSz(int c,int x,int y,byte sz)
{
  ...
  for(i=0;i<8;i++)
  {                                     
	  enlarge(large[i],pgm_read_byte(&(font[(8*c)+i])),sz);		//c here;GCC (c-1)
  }
  for(j=0;j<sz;j++)
  {
	  for(i=0;i<8;i++)
...
void glcd_putchar(byte c,int x,int y,byte l,byte sz)
{
    ...
    {       
        ...
        {
            putIt(pgm_read_byte(&(map[prevLet-193][stat+1])),prevX,prevY);
            stat = 2;
        }else stat = 0;
...
//------------BMP Display--------------------
void bmp_disp(const byte *bmp,unsigned int x1,unsigned int y1,
                              unsigned int x2,unsigned int y2)
{
    unsigned int i,j;    
    for(i=y1;i<=y2;i++)
